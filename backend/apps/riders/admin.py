...
# ==========================================
# ENTERPRISE RIDER ADMINS (STRICT ISOLATION)
# ==========================================
...
class RiderProfileAdmin(ImportExportModelAdmin):
    ...
    def get_queryset(self, request):
        ...
        # Isolate Riders based on selected session warehouse
        if wh_id: qs = qs.filter(current_warehouse_id=wh_id)
        else: return qs.none()
    ...
    def wallet_balance(self, obj):
        balance = (getattr(obj, 'annotated_earnings', 0) or 0) - (getattr(obj, 'annotated_payouts', 0) or 0)
        return f"₹{balance:.2f}"
    wallet_balance.short_description = "Wallet Balance"

    def assigned_warehouse(self, obj): return obj.current_warehouse.name if obj.current_warehouse else "Unassigned"
    assigned_warehouse.short_description = "Warehouse"

    def kyc_status(self, obj):
        if obj.is_kyc_verified: return format_html('<span style="color: green;">✓ Verified</span>')
        return format_html('<span style="color: orange;">⚠ Pending</span>')
    kyc_status.short_description = "KYC"

    def created_at_date(self, obj):
        if hasattr(obj, 'created_at') and obj.created_at: return obj.created_at.strftime('%d/%m/%Y')
        return "N/A"
...
class RiderPayoutAdmin(ImportExportModelAdmin):
    ...
    def get_queryset(self, request):
        qs = super().get_queryset(request)
        wh_id = request.session.get('selected_warehouse_id')
        if wh_id: return qs.filter(rider__current_warehouse_id=wh_id)
        return qs.none()
    ...
    def created_at_date(self, obj):
        if hasattr(obj, 'created_at') and obj.created_at: return obj.created_at.strftime('%d/%m/%Y %H:%M')
        return "N/A"
